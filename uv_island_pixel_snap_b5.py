...
def get_uv_islands(bm, uv_layer, use_sync, uv_select_mode):
    ...
    Blender 5: BMLoopUV.select no longer exists. Selection is read from
    mesh-level signals only: face.select, edge.select, vert.select.
    vert.select is the most granular reliable signal in Blender 5.
    """
    visited = set()
    islands = []

    # Blender 5.0+: loop.uv_select_vert is the per-corner UV selection signal,
    # replacing the removed BMLoopUV.select. It correctly distinguishes islands
    # in all UV select modes including sync off, unlike face.select which
    # spreads mesh-wide.
    test_loop = next((l for f in bm.faces for l in f.loops), None)
    has_uv_select_vert = (test_loop is not None and
                          hasattr(test_loop, 'uv_select_vert'))

    def face_has_selection(face):
        """
        True if this face contains any selected UV element.
        face.select is checked first as a staleness gate in all cases.
        Sync on: face.select is the authoritative signal — A key and other
                 mesh-level selections update it reliably.
        Sync off: uv_select_vert is the granular per-island signal — it
                  correctly distinguishes islands where face.select spreads
                  mesh-wide. face.select still gates against stale flags
                  from previously selected objects.
        """
        if not face.select:
            return False
        if use_sync:
            return True
        if has_uv_select_vert:
            return any(loop.uv_select_vert for loop in face.loops)
        return face.select
...
# ---------------------------------------------------------------------------
# Selection write-back — Blender 5
# ---------------------------------------------------------------------------

def apply_uv_selection(bm, uv_layer, loops, use_sync, uv_select_mode, select):
    """
    Blender 5: write selection via loop.uv_select_vert_set() when available,
    which is the proper B5 replacement for BMLoopUV.select.
    Falls back to face.select when unavailable or sync is on.
    """
    loops = list(loops)
    if not loops:
        return

    has_uv_select_vert = hasattr(loops[0], 'uv_select_vert')

    if has_uv_select_vert and not use_sync:
        for loop in loops:
            loop.uv_select_vert_set(select)
    else:
...
def snap_islands_to_pixel(image_width, image_height, snap_mode='CORNER',
    ...
    for bm, uv_layer, me in bm_map.values():
        # Sync on: push face.select into the UV editor via sync_from_mesh.
        # Sync off: uv_select_vert write-back handles selection directly;
        #           sync_from_mesh spreads selection mesh-wide so skip it.
        if use_sync and hasattr(bm, 'uv_select_sync_from_mesh'):
...
class UVPixelSnapProperties(PropertyGroup):
    ...
    merge_overlapping: BoolProperty(
        ...
        description=(
            "Treat selected overlapping islands as one unit — they share a combined "
            "bounding box center and move by the same offset"
        ),
        default=False,
    )
...
class UV_PT_island_pixel_snap(Panel):
    ...
    def draw(self, context):
        ...
        # Sync off works reliably in all UV select modes in B5.
